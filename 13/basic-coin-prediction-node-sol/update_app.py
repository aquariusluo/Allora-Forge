...
def generate_features_sol(data):
    ...
    # Lagged OHLC features (reduced to 5 lags to preserve data)
    for col in ["open", "high", "low", "close"]:
        for i in range(1, 6):
            features[f"{col}_SOLUSDT_lag{i}"] = data_8h[col].shift(i)
    
    # Volatility (standard deviation of close prices over 3 periods)
    features["volatility_SOLUSDT"] = data_8h["close"].rolling(window=3).std()
    
    # Moving averages (7-period and 21-period)
    features["ma7_SOLUSDT"] = data_8h["close"].rolling(window=7).mean()
    features["ma21_SOLUSDT"] = data_8h["close"].rolling(window=21).mean()
